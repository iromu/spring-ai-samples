...
public class DefaultAIController implements OpenAIController {
    ...
    public Flux<ChatStreamResponse> chatCompletions(ChatRequest request) {
        ...
        // Step 1: Role delta
        ChatStreamResponse roleChunk = new ChatStreamResponse(
                id, "chat.completion.chunk", created, model,
                new ChatStreamResponse.Choice[]{
                        new ChatStreamResponse.Choice(
                                new ChatStreamResponse.Delta("assistant", null),
                                0, null
                        )
                }
        );
        ...
        Flux<ChatStreamResponse> contentChunks = Flux.range(0, tokens.length)
                .map(i -> new ChatStreamResponse(
                        id, "chat.completion.chunk", created, model,
                        new ChatStreamResponse.Choice[]{
                                new ChatStreamResponse.Choice(
                                        new ChatStreamResponse.Delta(null, tokens[i] + " "),
                                        0, null
                                )
                        }))
        ...
        ChatStreamResponse doneChunk = new ChatStreamResponse(
                id, "chat.completion.chunk", created, model,
                new ChatStreamResponse.Choice[]{
                        new ChatStreamResponse.Choice(
                                new ChatStreamResponse.Delta(null, null),
                                0, "stop"
                        )
                }
        );
