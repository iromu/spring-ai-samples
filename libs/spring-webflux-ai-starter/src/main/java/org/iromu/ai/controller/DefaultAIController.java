...
public class DefaultAIController implements OpenAIController {
	...
	public Flux<ChatStreamResponse> chatCompletions(ChatRequest request) {
		...
		// Step 1: Role delta
		ChatStreamResponse roleChunk = new ChatStreamResponse(id, "chat.completion.chunk", created, model,
		...
		ChatStreamResponse doneChunk = new ChatStreamResponse(id, "chat.completion.chunk", created, model,
				new ChatStreamResponse.Choice[] {
